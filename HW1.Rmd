...
K = 266
# spaghetti plot
ggplot(data, aes(x = time, y = cd4, group = id, color = id)) + 
  geom_line()

# Stage 1
betaMat = data.frame(beta0=rep(NA, K), beta.time=rep(NA, K), beta.packs=rep(NA, K), 
                     beta.drug=rep(NA, K), beta.partners=rep(NA, K), beta.cesd=rep(NA, K))

for(k in 1:K) {
  temp.k = data[data$id == k,]
  fit.k = lm(cd4 ~ time + packs + drug + partners + cesd, data = temp.k)
  betaMat[k, 1:6] = c(fit.k$coef)
}
